...
public partial class ModWindow
{
    ...
    private void Window_Mod_ContentRendered(object sender, EventArgs e)
    {
        ...
        {
            LoggerHelper.Error($"{Lang.ModWin_ReadConfigException}  {ex.Message}");
        }
    }
    ...
    private void SelcetModsPath()
    {
        ...
        {
            Title = Lang.ModWin_FileDialogTitle,
            DefaultExt = ".fbmod",
            Filter = Lang.ModWin_FileDialogFilter,
            Multiselect = true,
        ...
        try
        {
            ...
            LoggerHelper.Info(Lang.ModWin_ClearOldMods);
        ...
        {
            LoggerHelper.Error($"{Lang.ModWin_SelectModsException}  {ex.Message}");
        }
    }
    ...
    private void RunBF1ModGame()
    {
        try
        {
            ...
            // 写入 Config\manager_config.json 配置文件
            FileHelper.WriteAllTextUtf8NoBom(CoreUtil.File_Config_ManagerConfig, JsonHelper.JsonSerialize(modConfig));
            LoggerHelper.Info(Lang.ModWin_WriteConfigFrostyModManager);

            LoggerHelper.Info(Lang.ModWin_LaunchFrostyModManager);
            ProcessHelper.OpenProcess(CoreUtil.File_Frosty_FrostyModManager, Globals.LocaleID);
        ...
        {
            LoggerHelper.Error($"{Lang.ModWin_InstallModsException}  {ex.Message}");
        }
    }
}