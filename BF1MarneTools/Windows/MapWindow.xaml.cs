...
public partial class MapWindow
{
    ...
    private async Task InitMapData()
    {
        ...
        {
            LoggerHelper.Error($"{Lang.MapWin_InitMapException}  {ex.Message}");
        }
    }
    ...
    private void SaveMapList()
    {
        try
        {
            ...
            LoggerHelper.Info(Lang.MapWin_SaveMapSuccess);
            LoggerHelper.Info($"{Lang.MapWin_MapListPath} {MapListFilePath}");
        }
        catch (Exception ex)
        {
            LoggerHelper.Error($"{Lang.MapWin_SaveMapListException}  {ex.Message}");
        }
