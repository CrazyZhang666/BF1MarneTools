...
public static class ProcessHelper
{
    ...
    public static void OpenLink(string url)
    {
        ...
        {
            LoggerHelper.Error($"打开http链接出现异常 {url}", ex);
        }
    }
    ...
    public static void OpenDirectory(string dirPath, bool isNotify = false)
    {
        ...
        {
            if (isNotify)
                NotifierHelper.Warning("文件夹路径不存在");
        ...
        {
            if (isNotify)
                NotifierHelper.Error("打开文件夹出现异常");

            LoggerHelper.Error($"打开文件夹出现异常 {dirPath}", ex);
        }
    }
    ...
    public static void OpenProcess(string appPath, string args = "", bool isSilent = false)
    {
        ...
        {
            LoggerHelper.Error($"启动程序出现异常 {appPath}", ex);
        }
    }
    ...
    public static void CloseProcess(string appName)
    {
        ...
        {
            LoggerHelper.Error($"关闭进程出现异常 {appName}", ex);
        }
    }
}