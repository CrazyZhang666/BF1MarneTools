...
public static class RegistryHelper
{
    ...
    public static string GetRegistryTargetVaule(string regPath, string keyName)
    {
        ...
        {
            LoggerHelper.Error($"读取注册表出现异常 {regPath} {keyName}", ex);
            return string.Empty;
        }
    }
    ...
    public static void SetRegistryTargetVaule(string regPath, string keyName, string value)
    {
        ...
        {
            LoggerHelper.Error($"写入注册表出现异常 {regPath} {keyName} {value}", ex);
        }
    }
    ...
    public static void CheckAndAddEaAppRegistryKey()
    {
        try
        {
            ...
            LoggerHelper.Info("写入注册表 EADesktop 虚拟安装路径成功");
        }
        catch (Exception ex)
        {
            LoggerHelper.Error("写入注册表 EADesktop 虚拟安装路径出现异常", ex);
        }
    }
}