...
public static class LocalHttpServer
{
    ...
    public static void Run()
    {
        ...
        // 配置服务器地址和端口号
        _httpListener.Prefixes.Add("http://127.0.0.1:65200/");
        // 启动服务器
        _httpListener.Start();
        // 开始监听
        ...
        LoggerHelper.Debug("Local Http 服务监听端口为 65200");
    }
        ...
        try
        {
            ...
            if (request.HttpMethod == "GET")
            {
                ...
                if (IsTrueRawUrl(request, "/RunBf1Game"))
                {
                    ...
                    // 这个相应内容 RUN_BF1_GAME_SUCCESS 不能随便修改
                    // 它和Mod安装工具里面对应的 FrostyModExecutor.cs

                    // 告诉客户端返回的ContentType类型为纯文本格式，编码为UTF-8
                    response.AddHeader("Content-type", "text/plain;charset=UTF-8");

                    response.OutputStream.Write(Encoding.UTF8.GetBytes("RUN_BF1_GAME_SUCCESS"));
