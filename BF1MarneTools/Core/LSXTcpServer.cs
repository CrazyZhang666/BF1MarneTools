...
public static class LSXTcpServer
{
    ...
    public static void Run()
    {
        ...
        {
            LoggerHelper.Warn("LSX 服务正在运行，请勿重复启动");
            return;
        }
    ...
    private static async void Result(IAsyncResult asyncResult)
    {
        ...
        try
        {
            ...
            while (client.Connected)
            {
                ...
                {
                    LoggerHelper.Error("处理 TCP Battlelog 客户端连接出现超时异常", ex);
                }
            }
        }
        catch (Exception ex)
        {
            ...
            if (!MakeGameClientCloseEx(ex))
                LoggerHelper.Error("处理 TCP 客户端连接出现异常", ex);
        }
        finally
        {
            client?.Close();
    ...
    private static async Task<string> ReadTcpString(TcpClient client, NetworkStream stream)
    {
        ...
        catch (Exception ex)
        {
            // 处理异常
            if (!MakeGameClientCloseEx(ex))
                LoggerHelper.Error("异步读取 TCP 字符串时出现异常", ex);
        }
