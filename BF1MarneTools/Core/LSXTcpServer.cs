...
public static class LSXTcpServer
{
    ...
    public static void Run()
    {
        ...
        {
            LoggerHelper.Warn(Lang.LSXTcpServer_NotAgainRun);
            return;
        }
        ...
        LoggerHelper.Info(Lang.LSXTcpServer_RunSuccess);
        LoggerHelper.Trace("LSX 服务端口为 3216");
    ...
    public static void Stop()
    {
        _tcpServer?.Stop();
        _tcpServer = null;
        LoggerHelper.Info(Lang.LSXTcpServer_StopSuccess);
    }
    ...
    private static bool MakeGameClientCloseEx(Exception ex)
    {
        ...
        LoggerHelper.Warn(Lang.LSXTcpServer_WarnGameClose);
        return true;
    }
    ...
    private static async void Result(IAsyncResult asyncResult)
    {
        ...
        try
        {
            ...
            LoggerHelper.Info($"{Lang.LSXTcpServer_ContentId} {contentId}");
            LoggerHelper.Info(Lang.LSXTcpServer_HandleLaunch);
            ...
            LoggerHelper.Trace($"处理解密 Challenge 响应 Seed {newResponse}");
            ...
            while (client.Connected)
            {
                ...
                {
                    LoggerHelper.Error(Lang.LSXTcpServer_HandleClientBattlelogConnectException, ex);
                }
            }
        }
        catch (Exception ex)
        {
            ...
            if (!MakeGameClientCloseEx(ex))
                LoggerHelper.Error(Lang.LSXTcpServer_HandleClientConnectException, ex);
        }
        finally
        {
            client?.Close();
    ...
    private static async Task<string> ReadTcpString(TcpClient client, NetworkStream stream)
    {
        ...
        catch (Exception ex)
        {
            // 处理异常
            if (!MakeGameClientCloseEx(ex))
                LoggerHelper.Error(Lang.LSXTcpServer_AsyncReadStringException, ex);
        }
    ...
    private static async Task<string> LSXRequestHandleForBFV(string request, string contentId)
    {
        ...
        LoggerHelper.Trace($"BFV LSX 请求 Request {request}");

        var partArray = request.Split('\"');
        LoggerHelper.Trace($"BFV LSX 请求 partArray 长度 {partArray.Length}");

        var id = partArray[3];
        var requestType = partArray[4];

        LoggerHelper.Trace($"BFV LSX 请求 Id {id}");
        LoggerHelper.Trace($"BFV LSX 请求 RequestType {requestType}");
