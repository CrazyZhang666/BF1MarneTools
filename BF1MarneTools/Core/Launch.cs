...
public static class Launch
{
    ...
    public static void RunBf1Game()
    {
        try
        {
            ...
            // 自定义战地1文档目录
            var diceDir = Path.Combine(CoreUtil.Dir_CommonAppData, "MARNE");
            ...
            if (!File.Exists(profSaveFilePath))
            {
                ...
                ResHelper.ExtractResFile("Data.PROFSAVE.zip", profSaveZip);
            ...
            {
                argsList.Add("-mserver -serverInstancePath ./Instance/");
            }
            else
            {
                // 是否显示Fps
            ...
            LoggerHelper.Info($"战地1进程Id为: {process.Id}");
