...
public static class CoreUtil
{
    ...
    /// 验证 Frosty 文件Crc32
    /// </summary>
    public static async Task<bool> VerifyFrostyCrc32()
    {
        var crc32_FrostyModManager = ResHelper.GetEmbeddedZipResFileCrc32("Data.Frosty.zip", "FrostyModManager.exe");
        var crc32_File = await FileHelper.GetFileCrc32(File_Frosty_FrostyModManager);

        return crc32_File == crc32_FrostyModManager;
    }

    /// <summary>
    /// 验证 Marne 文件Crc32
    /// </summary>
    public static async Task<bool> VerifyMarneCrc32()
    {
        var crc32_MarneDll = ResHelper.GetEmbeddedZipResFileCrc32("Data.Marne.zip", "Marne.dll");
        var crc32_File_MarneDll = await FileHelper.GetFileCrc32(File_Marne_MarneDll);
        if (crc32_MarneDll != crc32_File_MarneDll)
            return false;

        var crc32_MarneLauncher = ResHelper.GetEmbeddedZipResFileCrc32("Data.Marne.zip", "MarneLauncher.exe");
        var crc32_File_MarneLauncher = await FileHelper.GetFileCrc32(File_Marne_MarneLauncher);
        if (crc32_MarneLauncher != crc32_File_MarneLauncher)
            return false;
    ...
    /// 验证 Service 文件Crc32
    /// </summary>
    public static async Task<bool> VerifyServiceCrc32()
    {
        var crc32_EADesktop = ResHelper.GetEmbeddedZipResFileCrc32("Data.Service.zip", "EADesktop.exe");
        var crc32_File = await FileHelper.GetFileCrc32(File_Service_EADesktop);

        return crc32_File == crc32_EADesktop;
    }
}