...
public partial class MainWindow
{
    ...
    {
        LoggerHelper.Info(Lang.MainWin_RunMainAppSuccess);

        Title = $"{Lang.WindowTitle} v{CoreUtil.VersionInfo} - {CoreUtil.GetWorkMode()}";
    ...
    private void Window_Main_Closing(object sender, CancelEventArgs e)
    {
        ...
        if (!_isCodeClose)
        {
            ...
            if (!_isFirstNotice)
            {
                NotifyIcon_Main.ShowBalloonTip(Lang.WindowTitle, Lang.MainWin_NotifyBalloonMsg, BalloonIcon.Info);
                _isFirstNotice = true;
            }
        ...
        LoggerHelper.Info(Lang.MainWin_CloseSuccess);
    }
    ...
    {
        LoggerHelper.Info(Lang.MainWin_CheckNewVersion);
        NotifierHelper.Notice(Lang.MainWin_CheckNewVersion);
        ...
        for (int i = 0; i <= 4; i++)
        {
            ...
            if (i > 3)
            {
                ...
                LoggerHelper.Error(Lang.MainWin_CheckUpdateFailed);
                NotifierHelper.Error(Lang.MainWin_CheckUpdateFailed);
                return;
            }
            ...
            {
                // 最多重试3次
                LoggerHelper.Warn($"{Lang.MainWin_CheckUpdateRetry} {i}/3");
            }
            ...
            if (webVersion != null)
            {
                if (CoreUtil.VersionInfo >= webVersion)
                {
                    ...
                    LoggerHelper.Info($"{Lang.MainWin_CheckNoUpdate} v{CoreUtil.VersionInfo}");
                    NotifierHelper.Info($"{Lang.MainWin_CheckNoUpdate} v{CoreUtil.VersionInfo}");
                    return;
                }

                MainModel.IsNeedUpdate = true;
                LoggerHelper.Info($"{Lang.MainWin_CheckNeedUpdate} v{webVersion}");
                NotifierHelper.Warning($"{Lang.MainWin_CheckNeedUpdate} v{webVersion}");
                return;
            }
        }
    }
