...
public static class EaApi
{
    ...
    public static async Task<RespResult> GetLSXLicense(string email, string password, string requestToken, string contentId)
    {
        ...
        try
        {
            ...
            LoggerHelper.Info($"{respResult.ApiName} {Lang.EaApi_ResponseStatus} {response.ResponseStatus}");
            LoggerHelper.Info($"{respResult.ApiName} {Lang.EaApi_StatusCode} {response.StatusCode}");
            ...
            {
                LoggerHelper.Warn($"{respResult.ApiName} {Lang.EaApi_TimeOut}");
                return respResult;
            }
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (!string.IsNullOrWhiteSpace(decryptArray[1]))
                {
                    ...
                    LoggerHelper.Debug($"{respResult.ApiName} {Lang.EaApi_GetLicenseSuccess} {respResult.Content}");

                    respResult.IsSuccess = true;
                }
                else
                {
                    LoggerHelper.Warn($"{respResult.ApiName} {Lang.EaApi_GetLicenseFailed}");
                }
            }
            else
            {
                LoggerHelper.Warn($"{respResult.ApiName} {Lang.EaApi_RequestFailed} {response.Content}");
            }
        }
        catch (Exception ex)
        {
            respResult.Exception = ex.Message;
            LoggerHelper.Error($"{respResult.ApiName} {Lang.EaApi_Exception}", ex);
        }
