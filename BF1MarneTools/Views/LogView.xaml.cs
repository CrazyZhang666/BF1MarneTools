...
{
    public ObservableCollection<LogInfo> ObserColl_LogInfos { get; set; } = new();
    ...
    private void LogReceived(AsyncLogEventInfo logEventInfo)
    {
        ...
        {
            if (_maxRowCount > 0 && ObserColl_LogInfos.Count > _maxRowCount)
                ObserColl_LogInfos.RemoveAt(0);
