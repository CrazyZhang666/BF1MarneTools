...
public partial class LoadWindow
{
    ...
    private async void Window_Load_ContentRendered(object sender, EventArgs e)
    {
        ...
        await Task.Run(async () =>
        {
            try
            {
                ...
                // 检查 Frosty 资源文件
                if (await CoreUtil.VerifyFrostyCrc32())
                {
                ...
                else
                {
                    ...
                    ResHelper.ExtractResFile("Data.Frosty.zip", CoreUtil.File_Zip_Frosty);
                ...
                // 检查 Marne 资源文件
                if (await CoreUtil.VerifyMarneCrc32())
                {
                ...
                else
                {
                    ...
                    ResHelper.ExtractResFile("Data.Marne.zip", CoreUtil.File_Zip_Marne);
                ...
                // 检查 Service 资源文件
                if (await CoreUtil.VerifyServiceCrc32())
                {
                ...
                else
                {
                    ...
                    ResHelper.ExtractResFile("Data.Service.zip", CoreUtil.File_Zip_Service);
                ...
                AppendLogger("程序资源文件，全部初始化成功");
            }
            catch (Exception ex)
            {
                AppendLogger($"初始化过程中，出现异常：{ex.Message}");
            }
        });
    }
    ...
    private void SelcetBf1Path()
    {
        try
        {
            ...
            {
                AppendLogger("不支持的战地1游戏主程序版本，请选择正确的版本");
                return;
            }
            AppendLogger("战地1游戏主程序版本检查正常");
            ...
            AppendLogger("战地1所在磁盘格式检查正常");

            Globals.SetBF1AppPath(dialog.FileName);
            LoadModel.Bf1Path = dialog.FileName;

            AppendLogger("获取战地1游戏主程序路径成功");
            return;
        }
        catch (Exception ex)
        {
            LoggerHelper.Error("选择战地1安装路径-出现异常", ex);
            AppendLogger($"选择战地1安装路径-出现异常 {ex.Message}");
            return;
        }
    }
